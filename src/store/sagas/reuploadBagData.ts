...
const reuploadBagData = function*(action: {
  ...
  const document = bagsData[`${action.payload.registryUri}/${action.payload.bagId}`];
  if (!document) {
  ...
  if (!document.signatures["0"]) {
    ...
    i = '2'
    newBagId = `${action.payload.bagId.slice(0, action.payload.bagId.length - 1)} ${parseInt(i, 10) + 1}`
